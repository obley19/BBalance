...
## 4. SO SÁNH HIỆU QUẢ: BM25 vs VECTOR SEARCH vs HYBRID

### 4.1. Đặc điểm hiệu năng

| Tiêu chí | BM25 (Từ khóa) | Vector Search (Ngữ nghĩa) | Hybrid (Kết hợp) |
| :--- | :--- | :--- | :--- |
| **Tốc độ truy vấn** | 80–120ms | < 30ms | 100–150ms |
| **Cơ chế** | Khớp từ khóa + TF-IDF | Cosine Similarity trên embedding 768 chiều | RRF Fusion + Dynamic Weighting |
| **Điểm mạnh** | Chính xác với mã model, thông số kỹ thuật | Hiểu ngữ nghĩa, đồng nghĩa, paraphrase | Kết hợp ưu điểm cả hai |
| **Điểm yếu** | Không hiểu ngữ nghĩa ("áo ấm" ≠ "áo khoác") | Dễ nhầm các model gần nhau ("iPhone 15" ↔ "iPhone 14") | Tốc độ chậm nhất (chạy cả 2 pipeline) |

### 4.2. Phân tích theo kịch bản truy vấn

| Kịch bản | Ví dụ truy vấn | Phương pháp tốt nhất | Phân tích |
| :--- | :--- | :---: | :--- |
| **Tìm sản phẩm chính xác** | "iphone 15 pro max 256gb" | **BM25** | Mã model + dung lượng cụ thể → BM25 khớp chính xác từ khóa. Vector dễ nhầm "iPhone 15" với "iPhone 14" vì embedding gần nhau |
| **Tìm theo viết tắt** | "ip 15 prm" | **BM25** | Sau Query Expansion ("ip" → "iphone", "prm" → "pro max"), BM25 match chính xác. Vector không có cơ chế synonym riêng |
| **Tìm theo mô tả ngữ nghĩa** | "điện thoại chụp ảnh đẹp giá rẻ cho sinh viên" | **Vector** | Không có sản phẩm nào chứa đúng cụm "chụp ảnh đẹp giá rẻ cho sinh viên" → BM25 gần như thất bại. Vector hiểu ngữ nghĩa tổng thể |
| **Tìm theo danh mục chung** | "tai nghe bluetooth" | **Hybrid** | BM25 khớp "tai nghe bluetooth" trong title. Vector mở rộng thêm kết quả tương tự ("headphone wireless"). Hybrid gộp cả hai |
| **Truy vấn đa nghĩa** | "apple" | **Hybrid** | BM25 trả về mọi sản phẩm chứa "apple". Vector hiểu context e-commerce → ưu tiên Apple (thương hiệu). Hybrid cân bằng |
| **Tìm phụ kiện** | "ốp lưng iphone 14" | **BM25** | Cụm từ cụ thể, BM25 khớp chính xác title. Vector có thể trả về ốp lưng iPhone 13/15 do embedding gần nhau |

### 4.3. Kết luận so sánh

**BM25 hiệu quả hơn** trong các truy vấn chứa mã sản phẩm, thông số kỹ thuật, hoặc từ khóa chính xác — chiếm phần lớn kịch bản tìm kiếm trên sàn TMĐT. **Vector Search vượt trội** khi truy vấn mang tính mô tả ngữ nghĩa hoặc không chứa từ khóa chính xác trong tiêu đề sản phẩm.

Tuy nhiên, **Hybrid Search là phương pháp tối ưu nhất** cho hệ thống thực tế vì:

1. **Bao phủ toàn diện:** Kết hợp cả keyword matching lẫn semantic understanding, không bỏ sót kết quả ở bất kỳ kịch bản nào.
2. **Thích ứng tự động:** Dynamic Weighting tự phân tích query để điều chỉnh trọng số (0.8/0.2 cho query kỹ thuật, 0.2/0.8 cho query ngữ nghĩa), loại bỏ sự phụ thuộc vào một phương pháp duy nhất.
3. **Chống nhiễu:** Bộ lọc Cosine Similarity ≥ 0.55 loại bỏ "rác" ngữ nghĩa, trong khi RRF chỉ dùng thứ hạng thay vì điểm số gốc, miễn nhiễm với outlier từ BM25.

> **Nhận định:** Đối với bài toán tìm kiếm sản phẩm TMĐT tiếng Việt với 1.45 triệu sản phẩm, Hybrid Search (RRF + Dynamic Weighting) là lựa chọn tối ưu — đánh đổi ~30ms tốc độ bổ sung so với BM25 đơn thuần để đạt chất lượng kết quả toàn diện hơn đáng kể.
